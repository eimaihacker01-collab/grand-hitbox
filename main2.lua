...
local mainWindow = Library:CreateWindow("Kaktos mounia Hitbox Extender by realchris")
local mainTab = mainWindow:AddTab("Main")
local mainGroupbox = mainTab:AddLeftGroupbox("Hitbox")
...
mainGroupbox:AddSlider("extenderSize", { Text = "Size", Min = 2, Max = 150, Default = 10, Rounding = 1 }):OnChanged(updatePlayers)
...
miscGroupbox:AddLabel("Toggle UI"):AddKeyPicker("menuKeybind", { Default = "Ν", NoUI = true, Text = "Menu Keybind" })
...
local function addPlayer(player)
	...
	function playerIdx:UpdateESP()
		...
		if Toggles.espNameToggled.Value then
			local target = FindFirstChildMatching(playerChar, "UpperTorso") 
    or FindFirstChildMatching(playerChar, "LowerTorso")
    or playerChar:FindFirstChild("HumanoidRootPart")
			if target then
				...
				if vis then
					...
					nameEsp.Size = math.clamp(1000 / pos.Z + 10, 14, 50)
